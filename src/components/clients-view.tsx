...
export function ClientsView({ sessions, onReload, onOpenSession }: Props) {
  ...
  // `s.client === selected` exactly, so a meeting tagged "[scrubbed]"
  // exported into the "[scrubbed]" folder yet never appeared under "[scrubbed]".
  const norm = (v: string | null | undefined) => (v || "").trim().toLowerCase();

  // Client names actually used on sessions, deduped case-insensitively
  // so "[scrubbed]" and "[scrubbed]" can't become two rows for one real client.
  const taggedClients = useMemo(() => {
    ...
    for (const raw of sessions.map((s) => s.client).filter(Boolean)) {
      ...
      // so the sidebar shows "[scrubbed]" rather than "[scrubbed]" when both exist.
