...
for (const depth of [35, 50]) {
  test(`a meeting label at nesting depth ${depth} is reachable by the DOM scan`, () => {
    const leaf = el("div", {
      role: "button",
      "aria-label": "[scrubbed], 8:30 AM to 9:00 AM, Friday, August 14, 2026, Microsoft Teams Meeting, By Mark [scrubbed], Busy",
    });
    const chain = buildChain(depth, leaf);
    setDocument(doc([chain]));

    const { candidates, diag } = sandbox._calendarDomScanFunc();
    const found = candidates.find((c) => c.label.startsWith("[scrubbed]"));
    assert.ok(
      found,
      `expected the depth-${depth} "[scrubbed]" candidate to be found, got ` +
...
test("realistic single-day fixture: all 5 real meetings captured (was 1 of 5 pre-v1.2)", () => {
  const dayLabels = [
    "AWS Daily Pulse Call, 10:00 AM to 10:15 AM, Microsoft Teams Meeting, [scrubbed] [scrubbed]",
    "PRIORITY: AWS Sales| Active Project Status Reviews and Escalations, 10:00 AM to 10:30 AM, Microsoft Teams Meeting, Will [scrubbed]",
    "FW: AWS Connect - Italy / ECC next steps: weekly team connect, 11:30 AM to 12:00 PM",
    "AWS/[scrubbed] - IVA PoC Sync-up, 1:00 PM to 1:30 PM, Microsoft Teams Meeting",
  ...
  assert.ok(subjects.includes("AWS/[scrubbed] - IVA PoC Sync-up"));
...
const FIELD_LABELS = [
  "[scrubbed], 8:30 AM to 9:00 AM, Friday, August 14, 2026, Microsoft Teams Meeting, By Mark [scrubbed], Busy",
  "PRIORITY: AWS Sales| Active Project Status Reviews and Escalations, 10:00 AM to 10:30 AM, Friday, August 14, 2026, Microsoft Teams Meeting, By Will [scrubbed], Busy, Exception to recurring event",
  "Discuss the [scrubbed] collection script concerns and develop a path forward, 10:30 AM to 11:00 AM, Friday, August 14, 2026, Microsoft Teams Meeting, By [scrubbed], Bob Jr. [US-US], Busy",
  "[scrubbed]/AWS <> [scrubbed] | workforce management and quality management, 1:30 PM to 2:25 PM, Wednesday, August 12, 2026, By Justin [scrubbed], Busy",
  "AWS Daily Pulse Call , 9:30 AM to 9:45 AM, Friday, August 14, 2026, Microsoft Teams Meeting, By [scrubbed] [scrubbed], Busy, Recurring event",
  "Q3 Quarterly Management Meeting, 10:00 AM to 11:30 AM, Wednesday, August 12, 2026, By [scrubbed] Evite, Tentative",
  "Guardian/AWS/[scrubbed] Sync, 9:30 AM to 10:30 AM, Thursday, August 13, 2026, By [scrubbed], Ken, Busy",
];
...
test("FIELD: date after the time range resolves with NO column date", () => {
  ...
  const home = result.events.find((e) => e.subject === "[scrubbed]");
  assert.ok(home, "[scrubbed] missing");
  ...
  const [scrubbed] = result.events.find((e) => e.subject.startsWith("[scrubbed]/AWS"));
  assert.equal([scrubbed].start, "2026-08-12T13:30:00");
  const guardian = result.events.find((e) => e.subject === "Guardian/AWS/[scrubbed] Sync");
