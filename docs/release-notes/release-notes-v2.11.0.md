...
("[scrubbed] Training"), stopped it, immediately started a second meeting
("[scrubbed] Go Forward Sync"), and after both finished:

- The 56-min [scrubbed] recording's WAV had been overwritten with the
  [scrubbed] meeting's 32-min audio.
- [scrubbed]'s session JSON now showed `ended_at` equal to [scrubbed]'s stop
  time + audio_duration fields stamped from [scrubbed]'s recording, so
  the UI displayed *"You got 32 min of audio in a 93-min recording
  — about 61 min appears to be missing"*.
- [scrubbed]'s session JSON had [scrubbed]'s transcript and AI summary attached.

The smoking gun is in [scrubbed]'s per-session log:
...
[scrubbed]'s stop ran with `self._session.session_id = 56105413` —
[scrubbed]'s session id — so finalize wrote [scrubbed]'s 32-min audio to [scrubbed]'s
final WAV path and the sync-integrity stamping landed on [scrubbed].
...
but did NOT cover the recording-vs-processing race. While [scrubbed]'s
auto-process was still running in the background, the user started
the [scrubbed] meeting. [scrubbed]'s auto-process's `set_session([scrubbed])` had
aliased `_session` to [scrubbed]. The subsequent [scrubbed] `stop_recording()`
read the aliased `_session` and wrote everything to [scrubbed]. In
parallel, [scrubbed]'s `process_session()` body kept reading `self._session`
across its `await` points — by then `_session` had been reassigned
to [scrubbed] by the user starting the next meeting, so [scrubbed]'s transcript
ended up appended to [scrubbed]'s session object.
