...
>    meeting tagged `[scrubbed]` exported into the `[scrubbed]` folder but never
>    appeared under `[scrubbed]` in the app.
...
list matched exactly. A meeting tagged `[scrubbed]` therefore copied into the
`[scrubbed]` folder and then never showed up under `[scrubbed]` — the files were on
disk the whole time, just invisible in the app.

Client matching is now case- and whitespace-insensitive everywhere, and
`[scrubbed]` / `[scrubbed]` / `[scrubbed] ` collapse into one client instead of splitting
