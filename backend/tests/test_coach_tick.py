...
def test_empty_model_output_surfaces_error():
    ...
    s = _summarizer("")
    out = _run(s.coach_tick(
        segments=[{"speaker": "A", "text": "discussing the [scrubbed] endpoint"}]))
...
def test_total_output_capped_at_three():
    reply = (
        '{"clarifying_questions":["[scrubbed] rate limits and failover?",'
        '"India in-country carrier via which SBC?","Philippines Connect region?"],'
        '"risks":["Bedrock agent-core scope not pinned to a phase"],'
        '"follow_ups":["Name the [scrubbed] endpoint owner"]}'
    )
    s = _summarizer(reply)
    out = _run(s.coach_tick(
        segments=[{"speaker": "A", "text": "[scrubbed] India Philippines Bedrock [scrubbed]"}]))
