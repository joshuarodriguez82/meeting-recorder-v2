...
def test_round_trip_push_then_pull_client_identity_not_paths(tmp_path):
    ...
    2026-08-07's `G:\\My Drive\\Aon` incident."""
    archive = tmp_path / "archive"
    archive.mkdir()
    machine_a = tmp_path / "machine_a"
    machine_b = tmp_path / "machine_b"
    ...
    payload = {
        "acme": {
            "export_folder": r"G:\My Drive\Aon",
            "knowledge_folder": r"G:\My Drive\Aon\Docs",
            "display_name": "Acme",
        }
    }
...
def test_pull_preserves_local_folder_paths_even_when_archive_has_foreign_ones(tmp_path):
    ...
    _write(
        ...
        {
            "aon": {
                "export_folder": "/Users/josh/Documents/Aon",
                "knowledge_folder": "/Users/josh/Documents/Aon/Docs",
                "display_name": "Aon",
            }
        },
        mtime=1000,
    ...
    assert copied == ["client_configs.json"]
    local_data = json.loads((local / "client_configs.json").read_text())
    assert local_data["aon"]["export_folder"] == "/Users/josh/Documents/Aon"
    assert local_data["aon"]["knowledge_folder"] == "/Users/josh/Documents/Aon/Docs"
...
def test_pull_adds_archive_only_clients_with_empty_folder_fields(tmp_path):
    ...
    _write(
        local / "client_configs.json",
        {"aon": {"export_folder": "/local/Aon", "display_name": "Aon"}},
        mtime=1000,
    )
    _write(
        archive / "client_configs.json",
        {
            "aon": {"export_folder": "/local/Aon", "display_name": "Aon"},
            "ricoh": {
                "export_folder": r"G:\My Drive\Ricoh",
                "knowledge_folder": r"G:\My Drive\Ricoh\Docs",
                "display_name": "Ricoh",
            },
        },
        mtime=5000,
    ...
    assert copied == ["client_configs.json"]
    local_data = json.loads((local / "client_configs.json").read_text())
    assert local_data["aon"]["export_folder"] == "/local/Aon"
    assert local_data["ricoh"]["export_folder"] == ""
    assert local_data["ricoh"]["knowledge_folder"] == ""
    assert local_data["ricoh"]["display_name"] == "Ricoh"
...
def test_pull_never_deletes_local_client_missing_from_archive(tmp_path):
    ...
    _write(
        archive / "client_configs.json",
        {"aon": {"export_folder": "", "display_name": "Aon"}},
        mtime=5000,
    )
    copied = pull(str(local), str(archive))
    assert copied == ["client_configs.json"]
...
def test_pull_updates_display_name_from_newer_archive(tmp_path):
    ...
    _write(
        archive / "client_configs.json",
        {"aon": {"export_folder": "", "display_name": "AON (renamed)"}},
        mtime=5000,
    )
    copied = pull(str(local), str(archive))
    assert copied == ["client_configs.json"]
    local_data = json.loads((local / "client_configs.json").read_text())
    assert local_data["aon"]["display_name"] == "AON (renamed)"
...
def test_pull_does_not_update_display_name_from_older_archive(tmp_path):
    ...
    assert copied == []
    local_data = json.loads((local / "client_configs.json").read_text())
    assert local_data["aon"]["display_name"] == "Aon"
...
def test_sanitize_clears_windows_drive_letter_path(tmp_path, monkeypatch):
    ...
    for c in cleared:
        assert c["client"] == "Aon"
        assert c["old_value"] in (r"G:\My Drive\Aon", r"G:\My Drive\Aon\Docs")
...
def test_sanitize_clears_unc_path_on_posix(tmp_path, monkeypatch):
    ...
    _write(
        local / "client_configs.json",
        {"aon": {"export_folder": r"\\server\share\Aon", "display_name": "Aon"}},
    )
    cleared = sanitize_local_paths(str(local))
...
def test_sanitize_leaves_plausible_local_path_alone(tmp_path, monkeypatch):
    ...
    _write(
        local / "client_configs.json",
        {"aon": {"export_folder": "/Users/josh/Documents/Aon", "display_name": "Aon"}},
    )
    cleared = sanitize_local_paths(str(local))
    assert cleared == []
    data = json.loads((local / "client_configs.json").read_text())
    assert data["aon"]["export_folder"] == "/Users/josh/Documents/Aon"
...
def test_sanitize_leaves_existing_but_currently_missing_local_path_alone(tmp_path, monkeypatch):
    ...
    _write(
        local / "client_configs.json",
        {"aon": {"export_folder": "/Volumes/UnpluggedDrive/Aon", "display_name": "Aon"}},
    )
    cleared = sanitize_local_paths(str(local))
    assert cleared == []
    data = json.loads((local / "client_configs.json").read_text())
    assert data["aon"]["export_folder"] == "/Volumes/UnpluggedDrive/Aon"
...
def test_is_foreign_local_path_windows_dispatch(tmp_path, monkeypatch):
    ...
    assert _is_foreign_local_path("/Users/josh/Documents/Aon-does-not-exist") is True
    ...
    assert _is_foreign_local_path(r"C:\Users\josh\Documents\Aon") is False
...
def test_sanitize_returns_cleared_list_shape(tmp_path, monkeypatch):
    ...
    _write(
        local / "client_configs.json",
        {"ricoh": {"export_folder": r"G:\My Drive\Ricoh", "display_name": "Ricoh"}},
    )
    cleared = sanitize_local_paths(str(local))
    assert cleared == [{
        "client": "Ricoh",
        "field": "export_folder",
        "old_value": r"G:\My Drive\Ricoh",
    }]
