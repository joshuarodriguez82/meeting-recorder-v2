...
def test_file_paths_cannot_reach_the_file(event_log):
    ...
    events.emit(
        ...
        detail=r"C:\Users\[scrubbed]\AppData\Local\MeetingRecorder\x.wav",
        posix_detail="/Users/[scrubbed]/Library/Application Support/x.wav",
    )
    raw = event_log.read_text(encoding="utf-8")
    assert "[scrubbed]" not in raw
...
def test_finalize_failure_records_the_type_not_the_message(
    ...
    def fake_finalize(**kwargs):
        raise RuntimeError(
            r"could not write C:\Users\[scrubbed]\...\session_X.wav")
...
def test_document_indexing_counts_are_emitted_without_paths(
    ...
    report = {
        ...
        "skipped": [
            {"file": r"C:\Users\[scrubbed]\Docs\logo.png",
             "reason": "not a text document — images aren't indexed",
             "expected": True},
            {"file": r"C:\Users\[scrubbed]\Docs\model.csv",
             "reason": "unsupported file type: .csv", "expected": False},
            {"file": r"C:\Users\[scrubbed]\Docs\deck.pptx",
