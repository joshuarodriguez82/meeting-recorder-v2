...
def test_base_name_matches_export_service():
    ...
    for display, sid in (
        ...
        ("AWS [scrubbed] Italy Call 08032026", "DEF456"),
...
@pytest.mark.parametrize("a,b", [
    ("[scrubbed]", "[scrubbed]"), ("[scrubbed]", "[scrubbed]"), ("[scrubbed]", " [scrubbed] "), ("UPS", "ups"),
])
...
def test_sessions_for_client_matches_regardless_of_casing():
    ...
    case-insensitively (so files landed in G:\\My Drive\\[scrubbed]) while the
    UI filtered `s.client === "[scrubbed]"` case-sensitively (so those meetings
    were invisible under [scrubbed])."""
    rows = [
        _summary(session_id="1", client="[scrubbed]"),
        _summary(session_id="2", client="[scrubbed]"),
        _summary(session_id="3", client="[scrubbed] "),
        _summary(session_id="4", client="Acme"),
        _summary(session_id="5", client=""),
    ]
    got = {r["session_id"] for r in sessions_for_client(rows, "[scrubbed]")}
    assert got == {"1", "2", "3"}


def test_sessions_for_client_empty_name_matches_nothing():
    rows = [_summary(session_id="1", client="[scrubbed]"),
