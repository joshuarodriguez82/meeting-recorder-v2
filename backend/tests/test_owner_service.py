...
class TestSplitOwners:
    ...
    def test_comma_is_not_split(self):
        ...
        "McDonnell, Bob Jr. [US-US]" — so splitting on comma would
        shred a single name into two fake people. Do not add comma to
        the split pattern."""
        assert split_owners("McDonnell, Bob Jr. [US-US]") == [
            "McDonnell, Bob Jr. [US-US]"
        ]
...
class TestNormalizeOwner:
    ...
    def test_comma_preserved_within_a_single_piece(self):
        ...
        key, display = normalize_owner("McDonnell, Bob Jr.")
        assert display == "McDonnell, Bob Jr"  # trailing period stripped
        assert "," in key
