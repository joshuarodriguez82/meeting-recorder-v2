...
(`G:\\My Drive\\[scrubbed]`) roamed onto the Mac by whole-file copy, and the
Mac started repeatedly queuing exports against a drive letter that can
never exist there:

    Reconcile '[scrubbed]': queued 1 session(s) for export to G:\\My Drive\\[scrubbed]
...
# The JSON stores that live alongside the recordings dir but are not
# part of the session-JSON archive copy. Order doesn't matter — each is
...
SHARED_FILES = (
    CLIENT_CONFIGS_FILE, "summary_templates.json", "owner_aliases.json",
)
...
def sanitize_local_paths(recordings_dir: str) -> List[dict]:
    ...
    user's Mac already had `G:\\My Drive\\[scrubbed]` / `G:\\My Drive\\[scrubbed]`
