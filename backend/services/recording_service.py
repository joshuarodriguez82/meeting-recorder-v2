...
class RecordingService:
    ...
    def start_recording(
        ...
        # [scrubbed] 2h25m call — "you got 28 min of audio in a 149-min
    ...
    def stop_recording(self) -> Optional[Session]:
        ...
        # exact race that fired on 2026-06-15 when [scrubbed]'s auto-process
        # called set_session([scrubbed]) mid-[scrubbed]-recording), every later
        # `self._session` read would resolve to the WRONG session — and
        # `_build_audio_path(self._session.session_id)` then wrote
        # [scrubbed]'s audio to [scrubbed]'s WAV path, while `self._session.ended_at
        # = ...` stamped [scrubbed]'s JSON with [scrubbed]'s stop time. Both halves
    ...
    async def process_session(self, session: Optional[Session] = None) -> Session:
        ...
        # Field repro on 2026-06-15: [scrubbed] Training (56105413) had its
        # session_56105413.wav overwritten by the next meeting's 32-min
        # audio, and the [scrubbed] session ended up with [scrubbed]'s transcript +
        # summary attached. [scrubbed]'s session log carries the smoking gun:
        #   [stop] finalize_recording_streaming → ...\session_56105413.wav
        # written from inside [scrubbed]'s stop, because [scrubbed]'s `_session`
        # had been reassigned to [scrubbed] by the auto-process path that ran
        # while [scrubbed] was still recording.
        #
    ...
    def _fingerprint_speakers(
        ...
        [scrubbed]/[scrubbed] meeting overwrite happened. Safe to call when
